...
static void snd_pcm_substream_proc_hw_params_read(struct snd_info_entry *entry,
						  struct snd_info_buffer *buffer)
{
	printk("pcm.c snd_pcm_substream_proc_hw_params_read");
...
static void snd_pcm_substream_proc_sw_params_read(struct snd_info_entry *entry,
						  struct snd_info_buffer *buffer)
{
	printk("pcm.c snd_pcm_substream_proc_sw_params_read");
	struct snd_pcm_substream *substream = entry->private_data;
	struct snd_pcm_runtime *runtime;
...
static void snd_pcm_substream_proc_status_read(struct snd_info_entry *entry,
					       struct snd_info_buffer *buffer)
{
	printk("pcm.c snd_pcm_substream_proc_status_read");
	struct snd_pcm_substream *substream = entry->private_data;
	struct snd_pcm_runtime *runtime;
...
{
	printk("pcm.c snd_pcm_new_stream");
	int idx, err;
...
int snd_pcm_new_internal(struct snd_card *card, const char *id, int device,
	...
	struct snd_pcm **rpcm)
{
	printk("pcm.c snd_pcm_new_internal");
	return _snd_pcm_new(card, id, device, playback_count, capture_count,
			true, rpcm);
}
...
{
	printk("pcm.c snd_pcm_attach_substream");
	struct snd_pcm_str * pstr;
	struct snd_pcm_substream *substream;
	struct snd_pcm_runtime *runtime;
	struct snd_card *card;
...
{
    // 打印函数进入的信息，方便调试
    printk("pcm.c snd_pcm_dev_register");

    int cidx, err;                     // 定义索引和错误码
    struct snd_pcm_substream *substream; // PCM 子流结构指针
    struct snd_pcm *pcm;                // PCM 结构指针

    // 检查设备和设备数据是否有效，如果无效则返回错误
    if (snd_BUG_ON(!device || !device->device_data))
        return -ENXIO;                // 返回无设备错误

    pcm = device->device_data;         // 获取设备数据

    mutex_lock(&register_mutex);       // 锁定注册互斥量，以防并发访问
    ...
    for (cidx = 0; cidx < 2; cidx++) {
        int devtype = -1;              // 初始化设备类型
        if (pcm->streams[cidx].substream == NULL)
            continue;                  // 如果当前流没有子流，跳过

        // 根据当前索引确定设备类型
        switch (cidx) {
        case SNDRV_PCM_STREAM_PLAYBACK:  // 播放流
            devtype = SNDRV_DEVICE_TYPE_PCM_PLAYBACK; // 设置设备类型为播放
            break;
        case SNDRV_PCM_STREAM_CAPTURE:     // 捕获流
            devtype = SNDRV_DEVICE_TYPE_PCM_CAPTURE; // 设置设备类型为捕获
            break;
        }
        ...
        if (err < 0) {                    // 如果注册失败
            list_del_init(&pcm->list);   // 从列表中删除 PCM
            goto unlock;                 // 跳转到解锁部分
        }
 ...
 unlock:                                 // 解锁部分
    mutex_unlock(&register_mutex);       // 解锁互斥量
    return err;                          // 返回错误码（成功或失败）
}
...
static void snd_pcm_proc_read(struct snd_info_entry *entry,
			      struct snd_info_buffer *buffer)
{
	printk("pcm.c snd_pcm_proc_read");
	struct snd_pcm *pcm;
