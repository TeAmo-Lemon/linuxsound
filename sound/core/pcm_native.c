...
#include <linux/highmem.h> // 引入这个头文件以使用 kmap 和 kunmap
#include "pcm_local.h"
...
{
	// printk("pcm_native snd_pcm_info_user");
...
{
	printk("pcm_native hw_support_mmap");
...
static int fixup_unreferenced_params(struct snd_pcm_substream *substream,
				     struct snd_pcm_hw_params *params)
{
	printk("pcm_native.c fixup_unreferenced_params");
...
{
	// printk("pcm_native ssnd_pcm_hw_refine_user");
	struct snd_pcm_hw_params *params;
	int err;
...
static int snd_pcm_hw_params(struct snd_pcm_substream *substream,
			     struct snd_pcm_hw_params *params)
{
	printk("pcm_native.c snd_pcm_hw_params");
...
{
	printk("pcm_native.c snd_pcm_hw_free");
	struct snd_pcm_runtime *runtime;
	int result;
...
{
	printk("pcm_native snd_pcm_sw_params");
	struct snd_pcm_runtime *runtime;
	int err;
...
static int snd_pcm_do_start(struct snd_pcm_substream *substream,
			    snd_pcm_state_t state)
{
	printk("pcm_native.c snd_pcm_do_start");
...
static void snd_pcm_post_start(struct snd_pcm_substream *substream,
			       snd_pcm_state_t state)
{
	printk("pcm_native.c snd_pcm_post_start");
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static int snd_pcm_prepare(struct snd_pcm_substream *substream,
			   struct file *file)
{
	printk("pcm.native snd_pcm_prepare");
	int f_flags;
...
static int snd_pcm_do_drain_init(struct snd_pcm_substream *substream,
				 snd_pcm_state_t state)
{
	printk("pcm_native snd_pcm_do_drain_init");
	struct snd_pcm_runtime *runtime = substream->runtime;
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		switch (runtime->status->state) {
		case SNDRV_PCM_STATE_PREPARED:
...
{
	printk("pcm_native.c snd_pcm_hw_constraints_complete");
	struct snd_pcm_runtime *runtime = substream->runtime;
...
{
	printk("pcm_native snd_pcm_open_substream");
	struct snd_pcm_substream *substream;
	int err;
...
static int snd_pcm_open_file(struct file *file,
			     ...
			     int stream)
{
	printk("pcm_native.c snd_pcm_open_file");

	// 定义一个指向 `snd_pcm_file` 结构的指针，用于存储打开的 PCM 文件信息
	struct snd_pcm_file *pcm_file;
	
	// 定义一个指向 `snd_pcm_substream` 结构的指针，用于存储子流信息
	struct snd_pcm_substream *substream;

	int err;

	// 尝试打开一个子流，根据传入的 PCM 设备和流类型（播放/录音），分配并初始化 `substream`
	err = snd_pcm_open_substream(pcm, stream, file, &substream);
	if (err < 0)
		// 如果打开子流失败，直接返回错误代码
		return err;

	// 分配内存空间用于存储 `snd_pcm_file` 结构体，并初始化为 0
	// `GFP_KERNEL` 标志表示该分配发生在内核上下文中，允许阻塞
	...
	// 将已打开的子流存储在 `pcm_file` 结构中
	pcm_file->substream = substream;

	// 如果该子流的引用计数为 1，表示这是第一个用户，则设置子流的释放函数
	if (substream->ref_count == 1)
		substream->pcm_release = pcm_release_private;

	// 将 `pcm_file` 指针赋值给文件的私有数据字段，以便后续访问
...
{
	printk("pcm_native.c snd_pcm_playback_open");
	struct snd_pcm *pcm;
...
{
	printk("pcm_native.c snd_pcm_capture_open");
	struct snd_pcm *pcm;
	int err = nonseekable_open(inode, file);
	if (err < 0)
		return err;
...
{
    // 打印函数进入的信息，方便调试
    printk("pcm_native.c snd_pcm_open\n");

    // 打印当前文件的信息，包括文件标志、inode编号和文件名
    printk("File information: f_flags = 0x%x, inode = %lu, filename = %s\n", 
        file->f_flags,                   // 文件的打开标志
        file->f_inode->i_ino,           // 文件的inode编号
        file->f_path.dentry->d_name.name); // 文件名

    int err;                           // 用于存储错误码
    wait_queue_entry_t wait;           // 等待队列条目

    // 检查 pcm 是否为空，若为空则返回 -ENODEV 表示设备不存在
    if (pcm == NULL) {
        err = -ENODEV;                // 设备不存在错误
        goto __error1;               // 跳转到错误处理部分
    }

    // 将打开的文件添加到 PCM 设备的卡中
    err = snd_card_file_add(pcm->card, file);
    if (err < 0)
        goto __error1;               // 如果添加失败，跳转到错误处理

    // 尝试获取模块的引用，确保 PCM 设备不会被卸载
    if (!try_module_get(pcm->card->module)) {
        err = -EFAULT;               // 如果获取失败，返回错误
        goto __error2;               // 跳转到错误处理部分
    }

    // 初始化等待队列条目
    init_waitqueue_entry(&wait, current); // 将当前任务添加到等待队列
    add_wait_queue(&pcm->open_wait, &wait); // 将等待条目添加到 PCM 的等待队列
    ...
    while (1) {
        ...
        if (err == -EAGAIN) {
            if (file->f_flags & O_NONBLOCK) {
                err = -EBUSY;        // 如果设置了非阻塞标志，返回设备忙错误
                break;
            }
        } else
            break;                   // 如果是其他错误，退出循环

        // 设置当前任务状态为可中断，允许其他进程运行
        set_current_state(TASK_INTERRUPTIBLE);
        mutex_unlock(&pcm->open_mutex); // 解锁以允许其他任务访问

        // 调度其他任务
        schedule();
        mutex_lock(&pcm->open_mutex); // 重新加锁

        // 检查 PCM 设备的卡是否已关闭
        if (pcm->card->shutdown) {
            err = -ENODEV;            // 如果关闭，返回设备不存在错误
            break;
        }

        // 检查当前任务是否有信号挂起
        if (signal_pending(current)) {
            err = -ERESTARTSYS;       // 如果有信号，返回重启系统调用错误
            break;
        }
    }
    ...
    mutex_unlock(&pcm->open_mutex); // 解锁

    // 如果打开失败，进入错误处理
    if (err < 0)
        goto __error;

    return err; // 返回成功，或返回打开的文件句柄

__error: // 错误处理部分
    module_put(pcm->card->module); // 释放模块引用
__error2: // 如果 snd_card_file_add 失败
    snd_card_file_remove(pcm->card, file); // 从 PCM 卡中移除文件
__error1: // 如果 PCM 为空或添加失败
    return err; // 返回错误码
}
...
{
	printk("pcm_native forward_appl_ptr");
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static snd_pcm_sframes_t rewind_appl_ptr(struct snd_pcm_substream *substream,
					 ...
					 snd_pcm_sframes_t avail)
{
	printk("pcm_native rewind_appl_ptr");
	// printk("pcm_native.c rewind_appl_ptr");
	struct snd_pcm_runtime *runtime = substream->runtime;
	snd_pcm_sframes_t appl_ptr;
	int ret;
...
static snd_pcm_sframes_t snd_pcm_forward(struct snd_pcm_substream *substream,
					 snd_pcm_uframes_t frames)
{
	printk("pcm_native.c snd_pcm_forward");
	snd_pcm_sframes_t ret;
...
/**
 * snd_pcm_sync_ptr - Sync the PCM stream pointer state with user space.
 * @substream: The PCM substream that we are working on.
 * @_sync_ptr: Pointer to the user space structure containing sync parameters.
 *
 * This function retrieves synchronization information from user space,
 * updates the PCM stream control and status, and then writes back the
 * updated state to user space.
 *
...
{
	printk("pcm_native snd_pcm_sync_ptr");
	struct snd_pcm_runtime *runtime = substream->runtime; // 获取PCM运行时结构
	struct snd_pcm_sync_ptr sync_ptr; // 本地同步指针结构
	volatile struct snd_pcm_mmap_status *status; // 指向内存映射状态的指针
	volatile struct snd_pcm_mmap_control *control; // 指向内存映射控制的指针
	int err;

	memset(&sync_ptr, 0, sizeof(sync_ptr)); // 清零本地同步指针结构

	// 从用户空间获取同步标志
	if (get_user(sync_ptr.flags, (unsigned __user *)&(_sync_ptr->flags)))
		return -EFAULT; // 错误处理：如果获取失败，则返回EFAULT

	// 从用户空间复制控制信息
	if (copy_from_user(&sync_ptr.c.control, &(_sync_ptr->c.control), sizeof(struct snd_pcm_mmap_control)))
		return -EFAULT; // 错误处理：如果复制失败，则返回EFAULT
	
	status = runtime->status; // 获取运行时状态
	control = runtime->control; // 获取运行时控制信息

	// 如果设置了硬件同步标志，则执行硬件同步
	if (sync_ptr.flags & SNDRV_PCM_SYNC_PTR_HWSYNC) {
		err = snd_pcm_hwsync(substream);
		if (err < 0)
			return err; // 错误处理：如果同步失败，则返回错误
	}

	snd_pcm_stream_lock_irq(substream); // 锁定PCM流以防止中断

	// 根据标志决定如何处理应用程序指针
	if (!(sync_ptr.flags & SNDRV_PCM_SYNC_PTR_APPL)) {
		// 如果没有应用程序同步，应用新的应用程序指针
		err = pcm_lib_apply_appl_ptr(substream, sync_ptr.c.control.appl_ptr);
		if (err < 0) {
			snd_pcm_stream_unlock_irq(substream); // 解锁
			return err; // 错误处理：如果应用失败，则返回错误
		}
	} else {
		// 否则，将控制指针设置为当前的应用程序指针
		sync_ptr.c.control.appl_ptr = control->appl_ptr;
	}

	// 更新可用最小值
	if (!(sync_ptr.flags & SNDRV_PCM_SYNC_PTR_AVAIL_MIN))
		control->avail_min = sync_ptr.c.control.avail_min; // 设置可用最小值
	else
		sync_ptr.c.control.avail_min = control->avail_min; // 获取当前可用最小值
	...
	snd_pcm_stream_unlock_irq(substream); // 解锁PCM流
...
{
	printk("pcm_native.c snd_pcm_ioctl_sync_ptr_compat");
	struct snd_pcm_runtime *runtime = substream->runtime;
	...
	snd_pcm_stream_unlock_irq(substream);
	// 在函数中适当位置添加打印语句
	// printk("User space status information:\n");
	// printk("  State: %d\n", sstatus.state);
	// printk("  HW Pointer: %u\n", sstatus.hw_ptr);
	// printk("  Timestamp: %ld.%09ld\n", sstatus.tstamp.tv_sec, sstatus.tstamp.tv_nsec);
	// printk("  Suspended State: %d\n", sstatus.suspended_state);
	// printk("  Audio Timestamp: %ld.%09ld\n", sstatus.audio_tstamp.tv_sec, sstatus.audio_tstamp.tv_nsec);
	// printk("Application Pointer: %lu\n", scontrol.appl_ptr);
	// printk("Available Minimum: %lu\n", scontrol.avail_min);
...
{
	printk("pcm_native snd_pcm_xferi_frames_ioctl");
...
{
	printk("pcm_native snd_pcm_xfern_frames_ioctl");
...
static int snd_pcm_forward_ioctl(struct snd_pcm_substream *substream,
				 snd_pcm_uframes_t __user *_frames)
{
	printk("pcm_native snd_pcm_forward_ioctl");
	snd_pcm_uframes_t frames;
	snd_pcm_sframes_t result;
...
{
	// printk("pcm_native snd_pcm_common_ioctl 0x%x\n", cmd);
...
static long snd_pcm_ioctl(struct file *file, unsigned int cmd,
			  unsigned long arg)
{
	// printk("pcm_native snd_pcm_ioctl");
	struct snd_pcm_file *pcm_file;
...
{
	// printk("pcm_native snd_pcm_kernel_ioctl");
...
static ssize_t snd_pcm_read(struct file *file, char __user *buf, size_t count,
			    loff_t * offset)
{
	printk("pcm_native snd_pcm_read");
	struct snd_pcm_file *pcm_file;
	struct snd_pcm_substream *substream;
	struct snd_pcm_runtime *runtime;
	snd_pcm_sframes_t result;
...
{
	printk("pcm_native snd_pcm_write");
	struct snd_pcm_file *pcm_file;
	struct snd_pcm_substream *substream;
	struct snd_pcm_runtime *runtime;
	snd_pcm_sframes_t result;
...
{
	printk("pcm_native snd_pcm_readv");
	struct snd_pcm_file *pcm_file;
	struct snd_pcm_substream *substream;
	struct snd_pcm_runtime *runtime;
	snd_pcm_sframes_t result;
...
static __poll_t snd_pcm_poll(struct file *file, poll_table *wait)
{
	...
	case SNDRV_PCM_STATE_DRAINING:
		if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
			printk("pcm_native snd_pcm_poll");
			mask = ok;
			if (!avail)
				mask |= EPOLLERR;
		}
...
{
	printk("pcm_native snd_pcm_mmap_status_fault");
...
{
	printk("pcm_native snd_pcm_mmap_control_fault");
	struct snd_pcm_substream *substream = vmf->vma->vm_private_data;
	struct snd_pcm_runtime *runtime;
...
static int snd_pcm_mmap_control(struct snd_pcm_substream *substream, struct file *file,
				struct vm_area_struct *area)
{
	printk("pcm_native snd_pcm_mmap_control");
	long size;
	if (!(area->vm_flags & VM_READ))
		return -EINVAL;
	size = area->vm_end - area->vm_start;
...
{
	// printk("pcm_native.c snd_pcm_mmap_data_fault");
	struct snd_pcm_substream *substream = vmf->vma->vm_private_data;
	struct snd_pcm_runtime *runtime;
...
//     // 获取页面的虚拟地址
//     unsigned char *data = page_address(page);  // 使用 page_address 来获取虚拟地址
//     size_t page_size = PAGE_SIZE; // 页面大小

//     // 遍历页面中的每个字节并将前四位设置为0
//     size_t i;  // 在这里声明变量
//     for (i = 0; i < page_size; i++) {
//         data[i] = (data[i] & 0x0F); // 将前四位设为0
//     }
...
int snd_pcm_lib_default_mmap(struct snd_pcm_substream *substream,
			     struct vm_area_struct *area)
{
	printk("pcm_native.c snd_pcm_lib_default_mmap");
	area->vm_flags |= VM_DONTEXPAND | VM_DONTDUMP;
	if (!substream->ops->page &&
...
int snd_pcm_lib_mmap_iomem(struct snd_pcm_substream *substream,
			   struct vm_area_struct *area)
{
	printk("pcm.h snd_pcm_lib_mmap_iomem");
	struct snd_pcm_runtime *runtime = substream->runtime;
...
int snd_pcm_mmap_data(struct snd_pcm_substream *substream, struct file *file,
		      struct vm_area_struct *area)
{
	printk("pcm_native snd_pcm_mmap_data");
	struct snd_pcm_runtime *runtime;
	long size;
	unsigned long offset;
	size_t dma_bytes;
	...
	if (substream->ops->mmap){		
		printk("pcm_native.c snd_pcm_mmap_data > substream->ops->mmap(substream, area);");
		err = substream->ops->mmap(substream, area);
	}
	else{
		printk("pcm_native.c snd_pcm_mmap_data > snd_pcm_lib_default_mmap");
...
{
	printk("pcm_native snd_pcm_mmap");
	struct snd_pcm_file * pcm_file;
	struct snd_pcm_substream *substream;	
	unsigned long offset;
